...
// 舵机配置
static servo_config_t servo_config = {.pin = SERVO_PIN,
...
// 频率管理器配置
static frequency_manager_config_t freq_config = {.current_mode = FREQ_MODE_PERFORMANCE,  // FREQ_MODE_BALANCED
...
// WiFi配置
static wifi_manager_config_t wifi_config = {.ap_ssid = "ESP32-S3-AP",
...
static void core0_perfmon_task(void* arg) {
    ...
    int xtensa_perfmon_result =
        xtensa_perfmon_init(PERF_COUNTER_CORE0, select_value, mask_value, kernelcnt_value, tracelevel_value);
...
static void core1_perfmon_task(void* arg) {
    ...
    int xtensa_perfmon_result =
        xtensa_perfmon_init(PERF_COUNTER_CORE1, select_value, mask_value, kernelcnt_value, tracelevel_value);
    if (xtensa_perfmon_result == 0) {
...
void app_main(void) {
    ...
    if (xTaskCreatePinnedToCore(core0_perfmon_task, "core0_perfmon", 4096, NULL, tskIDLE_PRIORITY + 1,
                                &runtime_0_task_handle, 0) != pdPASS) {
        ESP_LOGE(TAG, "Failed to create Core0 performance monitoring task");
        return;
    }

    // 创建Core 1性能监控任务（运行在Core 1）
    if (xTaskCreatePinnedToCore(core1_perfmon_task, "core1_perfmon", 4096, NULL, tskIDLE_PRIORITY + 1,
                                &runtime_1_task_handle, 1) != pdPASS) {
