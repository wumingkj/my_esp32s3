...
// 默认配置
static ws2812_config_t default_config = {.pin = WS2812_PIN,
...
esp_err_t ws2812_led_init(const ws2812_config_t* config) {
    ...
    ESP_LOGI(TAG, "WS2812 LED initialized successfully, pin: %d, leds: %d", ws2812_config->pin,
             ws2812_config->num_leds);
...
esp_err_t ws2812_set_led_color(int led_index, rgb_color_t color) {
    ESP_RETURN_ON_FALSE(ws2812_config && ws2812_config->is_running, ESP_ERR_INVALID_STATE, TAG,
                        "WS2812 not initialized");
    ESP_RETURN_ON_FALSE(led_index >= 0 && led_index < ws2812_config->num_leds, ESP_ERR_INVALID_ARG, TAG,
                        "invalid LED index");
...
static void rainbow_effect_task_simple(void* pvParameters) {
    ...
    while (ws2812_config && ws2812_config->is_running) {
        ...
        hsv_color_t hsv = {.h = hue % 360, .s = 1.0, .v = 1.0};
        rgb_color_t rgb;
        hsv_to_rgb(hsv, &rgb);
...
static void breathing_effect_task_simple(void* pvParameters) {
    ...
    while (ws2812_config && ws2812_config->is_running) {
        rgb_color_t color = {.r = (uint8_t)(base_color.r * brightness),
...
esp_err_t ws2812_rainbow_effect(int speed) {
    ...
    *param = speed;
    ...
    esp_err_t ret =
        xTaskCreate(rainbow_effect_task_simple, "rainbow_effect", 4096, param, 3, &ws2812_config->effect_task);
    if (ret != pdPASS) {
        free(param);
...
esp_err_t ws2812_breathing_effect(rgb_color_t color, int speed) {
    ...
    *param = color;
    ...
    esp_err_t ret =
        xTaskCreate(breathing_effect_task_simple, "breathing_effect", 4096, param, 3, &ws2812_config->effect_task);
    if (ret != pdPASS) {
        free(param);
