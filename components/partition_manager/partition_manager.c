...
esp_err_t partition_manager_configure_power(power_management_config_t* config) {
    ...
    ESP_LOGI(TAG, "Power management configured successfully: max_freq=%dMHz, min_freq=%dMHz", config->max_freq_mhz,
             config->min_freq_mhz);
    return ESP_OK;
#else
...
void partition_manager_list_partitions(void) {
    ...
    while (it != NULL) {
        ...
        if (partition != NULL) {
            ESP_LOGI(TAG, "Label: %s, Type: %d, Subtype: %d, Offset: 0x%lx, Size: %lu bytes", partition->label,
                     partition->type, partition->subtype, (unsigned long)partition->address,
