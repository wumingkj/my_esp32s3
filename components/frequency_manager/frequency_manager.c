...
esp_err_t frequency_manager_init(frequency_manager_config_t* config) {
    ...
    // 验证频率值
    if (config->performance_freq < 80 || config->performance_freq > 240 || config->balanced_freq < 80 ||
        config->balanced_freq > 240 || config->power_save_freq < 80 || config->power_save_freq > 240) {
