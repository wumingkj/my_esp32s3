...
esp_err_t wifi_manager_scan_connected_devices(const char* ap_name) {
    ...
    for (int i = 0; i < sta_list.num; i++) {
        ...
        char mac_str[18];
        snprintf(mac_str, sizeof(mac_str), "%02X:%02X:%02X:%02X:%02X:%02X", sta->mac[0], sta->mac[1], sta->mac[2],
        ...
        if (name_ret != ESP_OK || strcmp(hostname, "unknown") == 0) {
            ...
            snprintf(hostname, sizeof(hostname), "device_%02X%02X%02X", sta->mac[3], sta->mac[4], sta->mac[5]);
        }
    ...
    if (event_base == WIFI_EVENT) {
        switch (event_id) {
            case WIFI_EVENT_AP_STACONNECTED: {
                ...
                char mac_str[18];
                snprintf(mac_str, sizeof(mac_str), "%02X:%02X:%02X:%02X:%02X:%02X", event->mac[0], event->mac[1],
                ...
                if (name_ret != ESP_OK || strcmp(hostname, "unknown") == 0) {
                    // 如果无法获取设备名称或名称为"unknown"，使用MAC地址后几位作为标识
                    snprintf(hostname, sizeof(hostname), "device_%02X%02X%02X", event->mac[3], event->mac[4],
                             event->mac[5]);
                }
...
static esp_err_t serve_file(httpd_req_t* req, const char* file_path, const char* content_type) {
    ...
    // 为静态文件设置缓存头（1小时缓存）
    if (strstr(content_type, "image") != NULL || strstr(content_type, "css") != NULL ||
...
static const httpd_uri_t login_get = {
    .uri = "/login", .method = HTTP_GET, .handler = login_get_handler, .user_ctx = NULL};
...
static const httpd_uri_t dashboard = {
    .uri = "/dashboard", .method = HTTP_GET, .handler = dashboard_get_handler, .user_ctx = NULL};

static const httpd_uri_t network = {
    .uri = "/network", .method = HTTP_GET, .handler = network_get_handler, .user_ctx = NULL};

static const httpd_uri_t controls = {
    .uri = "/controls", .method = HTTP_GET, .handler = controls_get_handler, .user_ctx = NULL};

static const httpd_uri_t account = {
    .uri = "/account", .method = HTTP_GET, .handler = account_get_handler, .user_ctx = NULL};

static const httpd_uri_t backup_restore = {
    .uri = "/backup-restore", .method = HTTP_GET, .handler = backup_restore_get_handler, .user_ctx = NULL};

static const httpd_uri_t css = {.uri = "/style.css", .method = HTTP_GET, .handler = css_get_handler, .user_ctx = NULL};

static const httpd_uri_t js = {.uri = "/common.js", .method = HTTP_GET, .handler = js_get_handler, .user_ctx = NULL};
...
static const httpd_uri_t assets = {
    .uri = "/assets/*", .method = HTTP_GET, .handler = assets_get_handler, .user_ctx = NULL};
...
static esp_err_t users_api_post_handler(httpd_req_t* req) {
    ...
    if (strcmp(action->valuestring, "add") == 0) {
        ...
        result = user_manager_update_user(username_json->valuestring, password ? password->valuestring : NULL,
...
static esp_err_t whitelist_api_post_handler(httpd_req_t* req) {
    ...
    if (strcmp(action->valuestring, "add") == 0) {
        result = whitelist_manager_add_mac(mac->valuestring, description ? description->valuestring : "");
    } else if (strcmp(action->valuestring, "delete") == 0) {
        // 使用正确的函数名
...
static const httpd_uri_t logout_get = {
    .uri = "/logout", .method = HTTP_GET, .handler = logout_get_handler, .user_ctx = NULL};

// 431错误处理URI配置
static const httpd_uri_t header_too_large = {
    .uri = "/header_too_large", .method = HTTP_GET, .handler = header_too_large_handler, .user_ctx = NULL};
...
static const httpd_uri_t users_api_get = {
    .uri = "/api/users", .method = HTTP_GET, .handler = users_api_get_handler, .user_ctx = NULL};

static const httpd_uri_t users_api_post = {
    .uri = "/api/users", .method = HTTP_POST, .handler = users_api_post_handler, .user_ctx = NULL};

// 白名单管理API URI配置
static const httpd_uri_t whitelist_api_get = {
    .uri = "/api/whitelist", .method = HTTP_GET, .handler = whitelist_api_get_handler, .user_ctx = NULL};

static const httpd_uri_t whitelist_api_post = {
    .uri = "/api/whitelist", .method = HTTP_POST, .handler = whitelist_api_post_handler, .user_ctx = NULL};
...
static const httpd_uri_t devices_api_get = {
    .uri = "/api/devices", .method = HTTP_GET, .handler = devices_api_get_handler, .user_ctx = NULL};
