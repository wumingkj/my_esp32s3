...
// 舵机控制状态
static servo_status_t servo_status = {
    .state = SERVO_STATE_UNINITIALIZED,
    .current_angle = 90,
    .target_angle = 90,
    .last_update_time = 0,
    .last_error = ESP_OK
...
static esp_err_t servo_control_set_angle_internal(int angle) {
    ...
    ESP_LOGD(TAG, "Servo angle set to %d° (pulsewidth: %dus, duty: %lu)", 
...
esp_err_t servo_control_smooth_move(int target_angle, int duration_ms, float acceleration)
{
    ...
    if (acceleration > 0.9f) acceleration = 0.9f;
...
esp_err_t servo_control_init(const servo_config_t *config)
{
    ...
    // 计算脉宽
    int pulsewidth = angle_to_pulsewidth(servo_status.current_angle, 
                                        current_config.min_pulsewidth, 
                                        current_config.max_pulsewidth);
...
esp_err_t servo_control_diagnostic_test(void) {
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        ESP_LOGI(TAG, "角度 %d度 -> 脉宽: %dus, 占空比: %lu", 
                 angle, pulsewidth, duty);
