...
esp_err_t keymanager_init(keymanager_handle_t* handle) {
    ...
    // 创建按键处理任务
    BaseType_t result = xTaskCreate(keymanager_task, "keymanager_task", 4096, manager, 5, &manager->task_handle);
...
esp_err_t keymanager_add_key(keymanager_handle_t handle, const key_config_t* config) {
    ...
    // 配置GPIO
    gpio_config_t io_conf = {.pin_bit_mask = (1ULL << config->pin),
...
esp_err_t keymanager_register_callback(keymanager_handle_t handle, key_event_callback_t callback, void* user_data) {
    if (handle == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
...
static esp_err_t keymanager_process_key(keymanager_t* manager, key_state_t* key) {
    ...
    if ((current_time - key->last_change_time) >= key->debounce_time) {
        if (key->current_state != key->stable_state) {
            ...
            // 生成按下/释放事件
            key_event_t event = {.pin = key->pin, .timestamp = current_time, .duration = 0};
                ...
                if (press_duration < key->long_press_time) {
                    ...
                    if (key->enable_double_click) {
                        ...
                        if (key->click_count == 1) {
                            ...
                            // 检测到双击，立即发送双击事件
                            key_event_t double_click_event = {.pin = key->pin,
                        ...
                        // 直接发送单击事件
                        key_event_t click_event = {.pin = key->pin,
                    ...
                    // 长按事件 - 在释放时触发
                    key_event_t long_press_event = {.pin = key->pin,
        ...
        if ((current_time - key->last_click_time) > key->double_click_time) {
            ...
            for (uint32_t i = 0; i < key->click_count; i++) {
                key_event_t click_event = {
                    .pin = key->pin, .type = KEY_EVENT_SINGLE_CLICK, .duration = 0, .timestamp = current_time};
                keymanager_send_event(manager, click_event);
            }
            key->click_count = 0;
            key->waiting_for_double_click = false;
    ...
    if (key->is_pressed) {
        ...
        if (key->repeat_time > 0 && (current_time - key->last_repeat_time) >= key->repeat_time) {
            key_event_t repeat_event = {.pin = key->pin,
